...
class site_navigation:
    ...
    def visit(self, new_page):
        ...
        # Sets the newly opened window as the current
...
# Language setting, the program starts in english and can be changed to Māori
language = "English"
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
def dashboard():
    ...
    dashboard_window.geometry("500x520+700+200")
    ...
    if language == "English":
        priority_label = tk.Label(dashboard_window, text = "Click 'Priority' to sort by urgency", font = ("Arial", 10))
    else:
        priority_label = tk.Label(dashboard_window, text = "Patohia te 'Maakete' ki te kōmaka ma te ohorere", font = ("Arial", 10))
    priority_label.pack()
    ...
    if language == "English":
        ...
        # runs a command when clicked. (lambda is like a mini function) that marks the sorting as false (low to high)
        dashboard_tree.heading("Priority", text = "Priority", command=lambda: sort_treeview("Priority", False))
    ...
    # Māori headings
        ...
        dashboard_tree.heading("Priority", text = "Whakaarotau", command=lambda: sort_treeview("Priority", False))
    ...
    # sets the current selected issue to be empty
    selected_issue = None
    ...
    def show_issue(event):
        ...
        # Function to set the tasks to complete
        def complete():
            # If nothing is currently selected, breaks the loop
            if selected_issue is None:
                return

            # Sets the status to be "complete" instead of "incomplete" as defined earlier
            selected_issue["status"] = "complete"

            # popup to show that the task has been marked as complete
            messagebox.showinfo("Thank You!", "Marked as complete!")
        ...
        for issue in current_issues:
            ...
            if issue["id"] == issue_id:
                ...
                # Makes the description uneditable again
                description_box.config(state = "disabled")

                # Button to mark issue as complete, only shows when an issue is selected
                if language == "English":
                    mark_complete = tk.Button(dashboard_window, text = "Mark Complete?", command = complete)
                else:
                    mark_complete = tk.Button(dashboard_window, text = "Tohu Kua oti?", command = complete)
                mark_complete.pack(pady = 10)
    ...
    def sort_treeview(column, reverse):
        ...
        # creates an empty list that stores the data
        data = []

        # iterates though every row currently in the treeview
        for row_id in dashboard_tree.get_children():

            # gets the value from the current selected row and column
            value = dashboard_tree.set(row_id, column)

            # stores the value and row id in the data list
            data.append((int(value), row_id))

        # sorts the list based on the column values, when reverse = true, highest to lowest - when reverse = false, lowest to highest
        data.sort(reverse = reverse)

        # iterates though all the stored data
        for index, (_, row_id) in enumerate(data):

            # moves each row into its new position
            dashboard_tree.move(row_id, "", index)
        
        # sorts the data again, this time in the reverse of whatever reverse is set to
        dashboard_tree.heading(column, command = lambda: sort_treeview(column, not reverse))
...
def report():
    ...
    priority_cb = ttk.Combobox(report_window, values = issue_priority, state = "readonly")
    if language == "English":
